...
// FUNCTION DECLARATIONS - MAIN MENU
document.addEventListener("DOMContentLoaded", function () {
    ...
    const descriptions = [infoDescription, compositionDescription, analysisDescription];

    // 🔹 Abrir menu e overlay
    if (menuButton && menuModal && menuOverlay) {
    ...
    if (closeMenuButton && menuModal && menuOverlay) {
        closeMenuButton.addEventListener("click", () => {
            menuModal.classList.add("hidden");
            menuOverlay.classList.add("hidden");
            resetMenu(); // Volta ao menu principal ao fechar
        });
    }

    // 🔹 Voltar para o menu principal ao clicar no botão de voltar dentro do menu
    if (menuBack) {
...
// 🔹 Função para exibir uma seção dentro do menu
function showDescription(title, description) {
    menuTitle.textContent = title;
    if (menuBack) {
        menuBack.classList.remove("hidden");
        menuBack.classList.add("show"); // Mostra o botão de voltar
    }
    ...
    if (menuDescription) menuDescription.classList.add("fade-in");
}

// 🔹 Função para voltar ao menu principal dentro do menu
function resetMenu() {
    menuTitle.textContent = "Menu Principal";
    if (menuBack) {
        menuBack.classList.add("hidden");
        menuBack.classList.remove("show"); // Esconde o botão de voltar
    }
    if (menuOptions) menuOptions.classList.remove("hidden");

    descriptions.forEach((desc) => {
        if (desc) desc.classList.add("hidden");
    });

    if (menuDescription) menuDescription.classList.remove("fade-in");
}
    // 🔹 Adicionando os eventos de clique para cada opção do menu SOMENTE SE OS ELEMENTOS EXISTIREM
    if (infoLink) {
        infoLink.addEventListener("click", () => showDescription("Contextualização", infoDescription));
    }
    if (compositionLink) {
        compositionLink.addEventListener("click", () => showDescription("Metodologia & Corpus", compositionDescription));
    }
    if (analysisLink) {
        analysisLink.addEventListener("click", () => showDescription("Análise", analysisDescription));
    }

    // 🔹 Garantir que o botão de voltar para a página de projetos **NUNCA** desapareça
    if (backButton) {
        backButton.style.display = "flex"; // Mantém sempre visível
    }
});

// FUNCTION DECLARATIONS - MAIN MENU - END
